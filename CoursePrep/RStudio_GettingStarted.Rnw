...
A list of all installed packaged is seen by selecting the \emph{Packages} tab \figrefp{fig:RStudioPackages}.  A package can be loaded by selecting the box to the left of the package name, though I suggest that packages be loaded via the \R{library()} function (to be discussed elsewhere) in an R script rather than through this dialog box.  Help for each package can be obtained by clicking on the name of package.  The help will then appear in the \emph{Help} tab.

\begin{figure}[htbp]
  \centering
    \includegraphics[width=3in]{Figs/RStudio_TabPackages.png}
    \caption{Example of the RStudio \emph{Packages} tab.}
